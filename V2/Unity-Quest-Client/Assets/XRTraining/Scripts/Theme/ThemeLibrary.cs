...
{
    /// <summary>
    /// Builds the three built-in palettes in code so the project runs with no
    /// pre-authored .asset files. In a shipping project a client would instead
    /// author a <see cref="ThemePalette"/> asset (Create ▸ XR Training ▸ Theme
    /// Palette) and drop it on the Bootstrap component — the runtime treats an
