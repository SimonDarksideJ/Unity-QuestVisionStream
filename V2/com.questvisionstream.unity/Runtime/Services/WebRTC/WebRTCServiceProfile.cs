...
namespace QuestVisionStream.Services
{
    ...
    public class WebRTCServiceProfile : BaseServiceProfile<IWebRTCTransportModule>
    {
        ...
        [SerializeField]
        [Tooltip("Flip frames vertically before sending so the stream arrives UPRIGHT (the GPU readback convention delivers them flipped on-device). Run the server with QVS_FLIP_VERTICAL=false when this is on.")]
        private bool flipStreamVertically = true;
