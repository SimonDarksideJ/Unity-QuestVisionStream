...
namespace QuestVisionStream.Core
{
    ...
    public static class UnlitMaterialFactory
    {
        ...
        public static Material Create()
        {
            ...
            {
                shaderLogged = true;
                Debug.LogWarning($"[QVS:Render] Resources material '{ResourceMaterialPath}' MISSING — fell back to Shader.Find('{(shader != null ? shader.name : "<null>")}'). This is the config that strips on device; run Tools > QuestVisionStream > Regenerate Detection Material.");
            }
