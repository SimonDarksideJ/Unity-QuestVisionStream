...
| [Training-Flow.md](Training-Flow.md) | **The training flow**: the Training State Service (queue of expected detection classes, JSON scenarios) and the Training Presentation Service (step form, hand menu, location indicators/labels) — how UX actions travel the same path as detections. |
| [Training-Configuration-Reference.md](Training-Configuration-Reference.md) | **The unified training configuration reference**: the scenario JSON wire format, state machine capabilities and API, aligned member-by-member against BOTH implementations — the C# `com.ethar.trainingstatemachine` package and its standalone Python export — plus the parity policy that keeps them in lockstep. |
| [Training-Builder.md](Training-Builder.md) | **The training builder**: author scenarios as mermaid diagrams (markdown ⇄ configuration, both directions), import from spreadsheet CSV, with a validation report on every conversion. One tool — the Python CLI — with scenario JSON as the interchange into the Unity asset's Import/Export JSON. |
